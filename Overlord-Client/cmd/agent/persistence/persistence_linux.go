...
func installSystemd(exePath string) error {
	...
	if activateLinuxPersistence {
		if err := activateSystemdUser(servicePath); err != nil {
			return fmt.Errorf("failed to activate systemd user service: %w", err)
		}
	}
...
func activateSystemdUser(servicePath string) error {
	serviceName := filepath.Base(servicePath)
	if out, err := runLinuxCommand("systemctl", "--user", "daemon-reload"); err != nil {
		return fmt.Errorf("systemctl --user daemon-reload: %w: %s", err, strings.TrimSpace(string(out)))
	}
	if out, err := runLinuxCommand("systemctl", "--user", "enable", servicePath); err != nil {
		return fmt.Errorf("systemctl --user enable: %w: %s", err, strings.TrimSpace(string(out)))
	}
	if out, err := runLinuxCommand("systemctl", "--user", "restart", serviceName); err != nil {
...
func cronMarker() string {
	return "# overlord-agent:" + binaryName()
}
